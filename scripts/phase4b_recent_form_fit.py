"""CLI for Phase 4b: per-sport recent-form ablation vs Phase 2 baseline.

Per Reese 2026-05-26 evening Phase 4b sign-off:
  - Reference: model fit with recent-form feature (beta_6 free)
  - Ablation: same fit but beta_6 constrained to 0
  ...
  - Halt at Phase 4b boundary regardless of outcome
  - No auto-promotion to Phase 4c
...
from engine.validator.runner_v2 import (
    Phase4bResult,
    run_phase4b_recent_form_ablation,
)
...
def _write_summary(run_dir: Path, result: Phase4bResult) -> None:
    (run_dir / "summary.json").write_text(
        json.dumps(_to_dict(result), indent=2, default=str)
    )


def _write_markdown(run_dir: Path, result: Phase4bResult, wall_time_sec: float) -> None:
    lines: list[str] = []
    lines.append(f"# Phase 4b Recent-Form Ablation - {result.config_label}")
    lines.append("")
    ...
    lines.append("")
    lines.append(f"Sports with significant recent-form lift (FDR-corrected at alpha=0.05): "
                 f"{result.n_significant_after_fdr} of {len(result.sports)}")
    lines.append("")

    lines.append("## Per-sport recent-form ablation results")
    lines.append("")
    lines.append("| Sport | beta_6 | Ref acc | Ablation acc | Acc lift (95% CI) | Brier lift (95% CI) | p (1-sided) | FDR sig |")
    lines.append("|---|---:|---:|---:|---:|---:|---:|:---:|")
    for sport_name, sr in sorted(result.sports.items()):
        ...
        lines.append(
            f"| {sport_name} | {sr.fit_baseline.coefficients['beta_6']:+.4f} | "
    ...
    for sport_name, sr in sorted(result.sports.items()):
        ...
        lines.append(f"")
        lines.append(f"|       | reference (with form) | ablation (beta_6=0) |")
        lines.append(f"|-------|---------:|---------:|")
    ...
    lines.append("")
    lines.append("Halt at Phase 4b boundary per Reese 2026-05-26 evening sign-off. "
                 "Explicit Phase 4c sign-off required before continuing.")
...
def main(argv: list[str] | None = None) -> int:
    ...
    p = argparse.ArgumentParser(prog="python scripts/phase4b_recent_form_fit.py")
    p.add_argument("--config-label", default="wf-phase4b-recent-form-ablation")
    ...
    print(f"[phase4b] config_label={args.config_label}")
    print(f"[phase4b] train={TRAIN_SEASONS} holdout={HOLDOUT_SEASONS} drop={DROP_SEASONS}")
    print(f"[phase4b] sports={sports or 'all'}")
    print(f"[phase4b] FDR alpha={args.fdr_alpha}")
    print()

    t0 = time.time()
    result = run_phase4b_recent_form_ablation(
    ...
    print(f"[phase4b] artifacts -> {run_dir}")
    print(f"[phase4b] wall-clock: {elapsed/60:.1f} min")
    print()
    print("=" * 60)
    print(f"Sports with significant recent-form lift after FDR "
          f"(alpha={args.fdr_alpha}): {result.n_significant_after_fdr} of {len(result.sports)}")
    print()
    for sport_name, sr in sorted(result.sports.items()):
        sig = "SIG" if sr.significant_after_fdr else "  -"
        print(f"  [{sig}] {sport_name:18} beta_6={sr.fit_baseline.coefficients['beta_6']:+.4f} "
              f"acc_lift={sr.accuracy_lift:+.4f} "
              f"[{sr.accuracy_lift_ci[0]:+.4f}, {sr.accuracy_lift_ci[1]:+.4f}] "
              f"p={sr.p_value_one_sided:.4f}")
    print()
    print("Halting at Phase 4b boundary - explicit Phase 4c sign-off required.")
    return 0

"""CLI for Phase 4d: per-sport Massey off/def (β₄) ablation.

Reese 2026-05-27 kickoff conditions:
  - Reference fit: β₃ pinned to 0 (Phase 4c disposition), β₄ free,
    β₆ free (Phase 4b promotion)
  - Ablation fit: β₃ AND β₄ pinned to 0, β₆ free
  - Per-sport paired-bootstrap 1000-resample CI on (acc, brier) deltas
  - BH-FDR α=0.05 across 8 per-sport tests
  - Halt at phase boundary regardless of outcome
  - No auto-chain into Phase 4e
...
def _write_summary(run_dir: Path, result: Phase4dResult) -> None:
...
def _write_markdown(
    ...
    lines: list[str] = []
    lines.append(f"# Phase 4d Massey Off/Def Ablation - {result.config_label}")
    lines.append("")
    ...
    lines.append("- β₃ PINNED to 0 in both reference and ablation (Phase 4c disposition)")
    lines.append("- β₆ FREE in both (Phase 4b promotion)")
    lines.append("- Reference: β₄ free (Massey LS signal active)")
    lines.append("- Ablation: β₄ pinned to 0 (Massey signal computed, coefficient masked)")
    lines.append("")
    lines.append(f"Sports with significant Massey lift (FDR-corrected at α=0.05): "
                 f"{result.n_significant_after_fdr} of {len(result.sports)}")
    lines.append("")

    lines.append("## Per-sport Massey ablation results")
    lines.append("")
    lines.append("| Sport | β₄ | Ref acc | Ablation acc | Acc lift (95% CI) | Brier lift (95% CI) | p (1-sided) | FDR sig | >2pp? |")
    lines.append("|---|---:|---:|---:|---:|---:|---:|:---:|:---:|")
    for sport_name, sr in sorted(result.sports.items()):
        ...
        lines.append(
            f"| {sport_name} | {sr.fit_baseline.coefficients['beta_4']:+.4f} | "
    ...
    if audit_triggered_sports:
        ...
        lines.append("Per Reese 2026-05-27 standing rule: any sport with >2pp accuracy lift "
                     ...
                     "production precompute, failure-mode checks including the NEW "
                     "Massey-LS-actually-produces-opponent-adjusted-off/def check) before "
                     "promote. The audit is surfaced automatically — no prompt required.")
        lines.append("")
    ...
    lines.append("")
    lines.append("Halt at Phase 4d boundary per Reese 2026-05-27 kickoff conditions. "
                 "Explicit Phase 4e sign-off required before continuing.")
    lines.append("")
    lines.append("Section 5 / mechanism-verification discipline applies. Any striking "
                 ...
                 "If Phase 4d lifts, the 'Massey LS extracts opponent-adjustment that β₆ "
                 "doesn't' claim must be code-cited at massey_od.py before any causal "
                 "story is published.")
...
def main(argv: list[str] | None = None) -> int:
    ...
    p = argparse.ArgumentParser(prog="python scripts/phase4d_massey_offdef_fit.py")
    p.add_argument("--config-label", default="wf-phase4d-massey-offdef-ablation")
    ...
    print(f"[phase4d] config_label={args.config_label}")
    print(f"[phase4d] train={TRAIN_SEASONS} holdout={HOLDOUT_SEASONS} drop={DROP_SEASONS}")
    print(f"[phase4d] sports={sports or 'all 8'}")
    print(f"[phase4d] FDR alpha={args.fdr_alpha}, audit threshold={args.audit_threshold}")
    print(f"[phase4d] β₃ pinned to 0 (Phase 4c disposition); β₄ ablation; β₆ free")
    print()

    t0 = time.time()
    result = run_phase4d_offdef_ablation(
    ...
    print(f"[phase4d] artifacts -> {run_dir}")
    print(f"[phase4d] wall-clock: {elapsed/60:.1f} min")
    print()
    print("=" * 70)
    print(f"Sports with significant Massey lift after FDR "
    ...
    for sport_name, sr in sorted(result.sports.items()):
        ...
        print(f"  [{sig}] {sport_name:18} beta_4={sr.fit_baseline.coefficients['beta_4']:+.4f} "
    ...
    if audit_triggered_sports:
        print(f"[phase4d] >2pp lift in: {audit_triggered_sports}")
        print(f"[phase4d] Replay audit fires automatically per standing rule.")
        print()

    print("Halting at Phase 4d boundary - explicit Phase 4e sign-off required.")
    return 0
